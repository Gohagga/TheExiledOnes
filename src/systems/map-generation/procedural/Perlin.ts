...
export class Perlin {
	...
	public OctavePerlin(x: number, y: number, z: number, octaves: number, persistence: number): number {
		let total = 0;
		let frequency = 1;
		let amplitude = 1;
	...
	private static readonly permutation: number[] = [ 151,160,137,91,90,15,					// Hash lookup table as defined by Ken Perlin.  This is a randomly
	...
	private static readonly p: number[] = []; 													// Doubled permutation to avoid overflow
	...
	public perlin(x: number, y: number, z: number): number {
        ...
        Log.Info(2.3)
		let xf = x-math.floor(x);								// We also fade the location to smooth the result.
        Log.Info(2.4)
		...
		let aaa = Perlin.p[Perlin.p[Perlin.p[    xi ]+    yi ]+    zi ];
		let aba = Perlin.p[Perlin.p[Perlin.p[    xi ]+this.inc(yi)]+    zi ];
		let aab = Perlin.p[Perlin.p[Perlin.p[    xi ]+    yi ]+this.inc(zi)];
		let abb = Perlin.p[Perlin.p[Perlin.p[    xi ]+this.inc(yi)]+this.inc(zi)];
		let baa = Perlin.p[Perlin.p[Perlin.p[this.inc(xi)]+    yi ]+    zi ];
		let bba = Perlin.p[Perlin.p[Perlin.p[this.inc(xi)]+this.inc(yi)]+    zi ];
		let bab = Perlin.p[Perlin.p[Perlin.p[this.inc(xi)]+    yi ]+this.inc(zi)];
		let bbb = Perlin.p[Perlin.p[Perlin.p[this.inc(xi)]+this.inc(yi)]+this.inc(zi)];
	
		let x1 = Perlin.lerp(Perlin.grad (aaa, xf  , yf  , zf),				// The gradient function calculates the dot product between a pseudorandom
					Perlin.grad (baa, xf-1, yf  , zf),				// gradient vector and the vector from the input coordinate to the 8
					u);										// surrounding points in its unit cube.
		let x2 = Perlin.lerp(Perlin.grad (aba, xf  , yf-1, zf),				// This is all then Perlin.lerped together as a sort of weighted average based on the faded (u,v,w)
	...
	public static grad(hash: number, x: number, y: number, z: number) {
